...
rad=st.sidebar.radio("Navigation",["Home","Electricty consumption","Air index","Temperature stats", "Ice coverage stats","Indian statewise electricity stats","CO2 stats (choose country from below)"])
...
if rad=="Home":
    ...
    st.markdown(""" 
It is a dashboard in real-time dedicated to the statistics and trends of the major cause for climatic change, 
which are:\n
...
Apart from all this, in near future we are planning to add section where there would be a bundle of tips and information on smart living, living offgrid and how you could limitize and reduce resource usage.\n
Why we NEED CLIMATE FLUCTUATION ONLOOKER (CFO)?\n
Currently, there is not any technology available that has the capability to reverse climate change. The only thing which can reverse the effect is the change in human practices, and to bring that change we want people to see the reality through data visualization. And this our application would be sufficient to do. 
Our application will help in making people realize the urgency for change.\n
\n\n
-------
\n\n
...
if rad=="Indian statewise electricity stats":
    ...
    """)
    sll=st.selectbox("Select Plot",['ALL','Avg. consumption','Usage'])
